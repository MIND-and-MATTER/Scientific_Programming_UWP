...
class SpaceStation:
    def __init__(self, which_canvas):
        ...
        self.ball = sphere(pos=self.person.pos + self.person.axis,
                           color=color.gray(10), size=2 * 0.2 * vector(1, 1, 1)
                           )
...
michael_birthday = 7
ryan_birthday = 1
omega = 2*pi / (michael_birthday/ryan_birthday)  # angular speed of space station; period of rotation is 2*pi/omega
deltat = 0.001 * 2 * pi / omega
v0 = omega * (station1.R - station1.h)
scalefactor = 5 / (omega * station1.R)
v1 = arrow(
    canvas=scene1,
    pos=station1.ball.pos,
    color=color.green,
    axis=vector(0, 0, 0),
    shaftwidth=0.4, visible=False
)

v2 = arrow(
    canvas=scene2,
    pos=station2.ball.pos,
    color=color.green,
    axis=vector(0, 0, 0),
    shaftwidth=0.4, visible=False
)
...
click1 = label(
    canvas=scene1,
    pos=vector(0.8 * station1.R, -1 * station1.R, 0),
    text="Click to\nstart over", visible=False
)

click2 = label(
    canvas=scene2,
    pos=vector(0.8 * station2.R, -1 * station2.R, 0),
    text="Click to\nstart over", visible=False
)
...
while True:
    ...
    while True:
        ...
        station1.ball.rotate(
            angle=omega * deltat,
            axis=vector(0, 0, 1),
            origin=vector(0, 0, 0)
        )
        
        station1.hull.rotate(
            angle=omega * deltat,
            axis=vector(0, 0, 1),
            origin=vector(0, 0, 0)
        )
